...
const char * llm_type_name(llm_type type) {
    switch (type) {
        ...
        case LLM_TYPE_28B_A3B:       return "28B.A3B";
        ...
        case LLM_ARCH_ERNIE4_5_VL_MOE:
            {
                ml.get_key(LLM_KV_ATTENTION_LAYERNORM_RMS_EPS, hparams.f_norm_rms_eps);
                ml.get_key(LLM_KV_EXPERT_FEED_FORWARD_LENGTH,        hparams.n_ff_exp);
                ml.get_key(LLM_KV_VISION_EXPERT_FEED_FORWARD_LENGTH, hparams.n_ff_v_exp, false);
                ml.get_key(LLM_KV_EXPERT_SHARED_FEED_FORWARD_LENGTH, hparams.n_ff_shexp, false);
                ml.get_key(LLM_KV_INTERLEAVE_MOE_LAYER_STEP,         hparams.n_moe_layer_step);
                ml.get_key(LLM_KV_LEADING_DENSE_BLOCK_COUNT,         hparams.n_layer_dense_lead);
                ...
                if (hparams.n_ff_v_exp == 0) {
                    hparams.n_ff_v_exp = 512;  // ERNIE-VL default
                }

                switch (hparams.n_layer) {
                    case 28: type = LLM_TYPE_28B_A3B; break;
                    default: type = LLM_TYPE_UNKNOWN;
                }
            } break;
            ...
            case LLM_ARCH_ERNIE4_5_VL_MOE:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        if (static_cast<uint32_t>(i) >= hparams.n_layer_dense_lead) { // MoE layers
                            int n_ff_exp = hparams.n_ff_exp;
                            int n_ff_v_exp = hparams.n_ff_v_exp;  // Vision expert intermediate size
                            ...
                            // Vision expert MoE tensors
                            layer.v_ffn_gate_inp  = create_tensor(tn(LLM_TENSOR_V_FFN_GATE_INP,  "weight", i), {n_embd, n_expert}, 0);
                            layer.v_ffn_exp_probs_b = create_tensor(tn(LLM_TENSOR_V_FFN_EXP_PROBS_B, "bias", i), {n_expert}, TENSOR_NOT_REQUIRED);
                            layer.v_ffn_gate_exps = create_tensor(tn(LLM_TENSOR_V_FFN_GATE_EXPS, "weight", i), {n_embd,   n_ff_v_exp, n_expert}, TENSOR_NOT_REQUIRED);
                            layer.v_ffn_down_exps = create_tensor(tn(LLM_TENSOR_V_FFN_DOWN_EXPS, "weight", i), {  n_ff_v_exp, n_embd, n_expert}, 0);
                            layer.v_ffn_up_exps   = create_tensor(tn(LLM_TENSOR_V_FFN_UP_EXPS,   "weight", i), {n_embd,   n_ff_v_exp, n_expert}, 0);
...
ggml_cgraph * llama_model::build_graph(const llm_graph_params & params) const {
    ...
    switch (arch) {
        ...
        case LLM_ARCH_ERNIE4_5_VL_MOE:
            {
                llm = std::make_unique<llm_build_ernie4_5_vl_moe>(*this, params);
            } break;
        case LLM_ARCH_HUNYUAN_MOE:
            {
