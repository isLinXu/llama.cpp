...
llm_build_ernie4_5_vl_moe::llm_build_ernie4_5_vl_moe(const llama_model & model, const llm_graph_params & params) :
    ...
    // Get MROPE sections from hparams
    // ERNIE-VL uses [22, 22, 20, 0] for (n_h, n_w, n_t, extra)
    // With ERNIE3D rope type: interleaved [h,w,h,w,...,t,t,...] frequency layout
    int sections[4];
    std::copy(std::begin(hparams.rope_sections), std::end(hparams.rope_sections), sections);
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            // ERNIE-VL uses ERNIE3D RoPE (NORMAL rotation + interleaved 3D frequency)
            // sections [22, 22, 20, 0]: n_h=22, n_w=22, n_t=20
            // Frequency layout: [h0,w1,h2,w3,...,h42,w43,t44,...,t63]
            // Position slots: [t, h, w, 0]
        ...
        if (!is_moe_layer) {
            ...
            if (ubatch.embd) {
                // Vision tokens: use vision MoE experts
                ...
                // Text tokens: use text MoE experts
                moe_out = build_moe_ffn(cur,
